...
*/
import { ASTPath, Collection, JSCodeshift, Transform } from 'jscodeshift/src/core';
import fs from 'fs';
...
/**
 * Recursively traverses up the AST tree from a given node.
 *
 * @param {Object} path - The current AST path node.
 * @param {Function} callback - The callback function to execute at each level.
 * @param {Function} stopPredicate - A function to determine when to stop recursion.
 * @param {number} [depth=0] - The current depth in the tree (for debugging purposes).
 * @returns {boolean} - Returns true if the predicate was matched, false otherwise.
 */
function traverseUpAST(path, callback, stopPredicate, depth = 0) {
  if (!path || !path.parentPath || !path.parentPath.value) return false;

  // Execute the callback function on the current node
  callback(path, depth);

  // Check if the stopPredicate function returns true
  if (stopPredicate(path)) {
    return true; // Stop recursion and return true if the predicate is matched
  }

  // Recursively traverse up to the parent path
